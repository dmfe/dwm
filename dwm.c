...
buttonpress(XEvent *e)
{
    ...
    if((m = wintomon(ev->window)) && showsystray && m == systraytomon(m))
        stw = getsystraywidth();
    ...
    if (ev->window == selmon->barwin) {
        ...
        if (i < LENGTH(tags)) {
            ...
            click = ClkLtSymbol;
        else if (ev->x > selmon->ww - statusw - stw - sp) {
            x = selmon->ww - statusw - stw - sp;
            click = ClkStatusText;
            statussig = 0;
    ...
    if (m == selmon) { /* status is only drawn on selected monitor */
        ...
        for (text = s = stext; *s; s++) {
            if ((unsigned char)(*s) < ' ') {
                ...
                *s = '\0';
                sw = TEXTW(text) - lrpad;
                drw_text(drw, m->ww - statusw + x - stw - 2 * sp, 0, sw, bh, 0, text, 0);
                x += sw;
                *s = ch;
                text = s + 1;
            }
...
setup(void)
{
    ...
    updategeom();
    sp = sidepad;
    vp = (topbar == 1) ? vertpad : - vertpad;/* init atoms */
    ...
    xatom[XembedInfo] = XInternAtom(dpy, "_XEMBED_INFO", False);/* init cursors */
    ...
    for (i = 0; i < LENGTH(colors); i++)
        scheme[i] = drw_scm_create(drw, colors[i], 3);/* init system tray */
    updatesystray();
    /* init bars */
    updatebars();
    updatestatus();
